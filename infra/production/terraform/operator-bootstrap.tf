...
# Also grant Secrets User so it can read the postgres-admin-url secret it needs
# to connect to PostgreSQL for the embedding profile insertion.
resource "azurerm_role_assignment" "operator_bootstrap_keyvault_user" {
  count                = var.enable_operator_bootstrap ? 1 : 0
  scope                = azurerm_key_vault.projectx.id
...
# Store the PostgreSQL admin connection URL in Key Vault so the bootstrap
# container can access it without exposing the password in Terraform variables.
# The password is already generated by Terraform; this only moves it into a
# secret that the bootstrap identity can read and then remove after bootstrap.
resource "azurerm_key_vault_secret" "postgres_admin_url" {
  count        = var.enable_operator_bootstrap ? 1 : 0
  name         = "postgres-admin-url"
  key_vault_id = azurerm_key_vault.projectx.id
  value        = "postgresql://${var.postgresql_admin_user}:${urlencode(random_password.pg_admin.result)}@${azurerm_postgresql_flexible_server.projectx.fqdn}:5432/${azurerm_postgresql_flexible_server_database.projectx.name}?sslmode=require"
}
...
resource "azurerm_container_app" "operator_bootstrap" {
  ...
  template {
    container {
      ...
      env {
        ...
        secret_name = "postgres-admin-url"
      }
  ...
  secret {
    name                = "postgres-admin-url"
    key_vault_secret_id = azurerm_key_vault_secret.postgres_admin_url[0].id
  ...
  depends_on = [
    ...
    azurerm_role_assignment.operator_bootstrap_keyvault_user,
  ]
}
